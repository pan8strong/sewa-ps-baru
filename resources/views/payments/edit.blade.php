        ...
        <div class="bg-white p-6 rounded shadow">
            <form action="{{ route('payments.update', $payment->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="amount_paid" id="amount_paid" min="0" value="{{ old('amount_paid', $payment->amount_paid) }}" class="w-full border border-gray-300 rounded px-3 py-2" required>
                    @error('amount_paid')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="paid_at" id="paid_at" value="{{ old('paid_at', $payment->paid_at->format('Y-m-d')) }}" class="w-full border border-gray-300 rounded px-3 py-2" required>
                    @error('paid_at')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
